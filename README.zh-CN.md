...
这是一个专为 **ESP32-C3** 设计的开源 PWM 控制器项目。它通过简洁的 Web 界面提供友好的交互体验，支持实时调节占空比、查看风扇转速，并管理多个 PWM 设备。特别适合用于 DIY 机箱散热控制、智能家居通风系统等场景。
...
- 💾 **掉电保存**：设备配置、WiFi 参数和网页登录配置持久化存储在 NVS 中。
- 🌐 **双模式 WiFi**：非阻塞 STA/AP 状态机；重配和断线时保留带密码的救援热点。
- 🌍 **双语界面**：`/` 默认显示英文，`/zh` 显示简体中文。
- 📱 **响应式 UI**：移动端优先设计，支持系统明暗外观。
...
- 标准 4 线风扇 PWM 输入可能内部上拉到 5V，ESP32-C3 GPIO 并非 5V 容忍。请使用 NPN 三极管或小信号 NMOS 构成开集电极/开漏驱动，并配置基极/栅极下拉电阻，不要把风扇 PWM 线直接接到 ESP32。
- 外部低端驱动会反相信号，应在设备配置中勾选“外部驱动反相”。
- Tach 通常是开集电极输出，建议使用 4.7kΩ～10kΩ 电阻上拉到 ESP32 的 3.3V，禁止上拉到 12V。默认按每转 2 个脉冲计算，也可在页面配置为 1～8。
- PWM 默认允许 GPIO0、1、3～8、10、18、19；RPM 额外允许 GPIO2 输入。
- GPIO9 影响下载模式、GPIO20/21 用于串口，因此固件仍会拒绝这些必要风险引脚。
- GPIO8 同时连接板载 RGB LED，作为 PWM/RPM 使用时板载灯可能异常闪烁；GPIO2 仅建议作为外部上拉到 3.3V 的 RPM 输入。
- GPIO18/19 默认承担 USB-JTAG；将其用于 PWM/RPM 会关闭对应 USB-JTAG 功能，如依赖原生 USB 调试，请优先使用其他允许引脚。
...
5. 点击 **Upload Filesystem Image** 将 `data/` 目录下的 Web 资源上传至 LittleFS，或执行：
...
> [!IMPORTANT]
> 固件和 LittleFS 需要分别烧录。页面或翻译有更新时，必须重新上传文件系统镜像。

### 初次配置

1. 上电后会开启 `PWM-Controller-XXXXXX` 救援热点，默认密码为 `pwm-controller`。
2. 手机或电脑连接热点；访问 `http://192.168.4.1` 使用英文，访问 `http://192.168.4.1/zh` 使用简体中文。
3. 浏览器认证用户名为 `admin`、密码为 `pwm-controller`；摘要认证覆盖页面和全部 API。
4. 在“系统设置”页面修改网页登录凭据，并配置 WiFi SSID 和密码。
5. 在主界面点击“添加设备”，配置风扇名称和对应的 PWM/RPM 引脚。

> 摘要认证可阻止一般的局域网未授权操作，但 HTTP 本身不提供传输加密。不要把该管理端口直接暴露到互联网，首次登录后建议立即修改默认凭据。

## 📡 API 文档

后端提供 RESTful API 接口，可用于自动化集成。JSON 状态响应包含稳定的 `code`，以及默认英文的 `message` 或 `error` 文本。

| 路径 | 方法 | 描述 |
| :--- | :--- | :--- |
| `/api/devices` | GET | 获取所有设备列表及实时转速 |
| `/api/devices` | POST | 添加设备并持久化 |
...
| `/api/system/info` | GET | 获取 IP 地址、运行时间、固件版本等系统信息 |
| `/api/system/reset` | POST | 清空设备、WiFi 与登录配置并重启 |

## 🏗️ 项目结构

- `src/`：使用 Arduino 框架的 C++ 固件源码。
- `data/`：存放在 LittleFS 中的 HTML、CSS、JavaScript 和翻译资源。
- `platformio.ini`：PlatformIO 构建配置和项目版本号。
...
欢迎提交 PR 或在 [GitHub Issues](https://github.com/ShouChenICU/PWM-Controller/issues) 中反馈问题。
